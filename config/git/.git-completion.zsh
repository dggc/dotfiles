...
_git-checkout () {
  ...
  case $state in
    (branch-or-tree-ish-or-file)
      # TODO: Something about *:: brings us here when we complete at â€œ-â€.  I
      # guess that this makes sense in a way, as we might want to treat it as
      # an argument, but I canâ€™t find anything in the documentation about this
      # behavior.
      [[ $line[CURRENT] = -* ]] && return
...
_git-commit () {
  ...
  # TODO: --interactive isnâ€™t explicitly listed in the documentation.
  _arguments -w -S -s \
    ...
    (option-or-argument)
      ...
      case $line[1] in
        (list|show|edit|remove)
          ...
          ;;
        (add)
          # TODO: Only complete commits that donâ€™t have notes already, unless
  ...
  case $state in
    ...
    (option)
      ...
      if compset -P '[^.]##.*.'; then
        ...
        # following functions donâ€™t generate any output in the case of
        # multi-level options.
        case $IPREFIX in
          (alias.)
            __git_aliases && ret=0
      ...
      (option-or-argument)
        ...
        case $line[1] in
          ...
          (delete)
            ...
            case $state in
              (reflog-entry)
                # TODO: __git_ignore_line doesnâ€™t work here for some reason.
...
_git-rerere () {
  ...
  case $state in
    (command)
      # TODO: This isnâ€™t optimal, as forget get confused.
      _values command \
...
_git-diff-index () {
  ...
  # TODO: Description of -m doesnâ€™t match that for git-rev-list.  Whatâ€™s going
  # on here?
  # TODO: With --cached, shouldnâ€™t we only list files changed in index compared
...
# NOTE: git-parse-remote isnâ€™t a user command.
...
# NOTE: git-sh-setup isnâ€™t a user command.
...
__git_is_committish_range () {
  # TODO: This isnâ€™t quite right.  We would like to do parts=${(~s:..(.|))},
  # but that doesnâ€™t work.  (This would allow us to make sure that parts only
...
__git_files_relative () {
  ...
  # Collapse â€œ//â€ and â€œ/./â€ into â€œ/â€. Strip any remaining â€œ/.â€ and â€œ/â€.
  for file in ${${${${${(0)1}//\/\///}//\/.\///}%/.}%/}; do
    integer i n
...
__git_setup_revision_options () {
  ...
  revision_options=(
    ...
    # TODO: You canâ€™t actually specify --format without a format.
...
# current value can be suffixed with â€œ (current)â€.
#
# The second argument is the default value, so that the description of the
# default value can be suffixed with â€œ (default)â€.
#
# The third argument is the description to use for the true and false values.
#
# The rest of the arguments can be used to provide additional â€œbooleanâ€ values
...
# Now, for the main driverâ€¦
_git() {
  if (( CURRENT > 2 )); then
    local -a aliases
    local -A git_aliases
    # TODO: Should use -z here, but I couldnâ€™t get it to work.
